...
çalışır. Kirli bardak "CLEAR THE GLASS", lavabo "POUR IT AWAY",
...
dokümanları düzenle kaldır, arkada konuşma sesleri olmasın. İyi siparişte alkış sesi yerine ufak sevinme sesleri
olabilir." ve sonra: "insan sesi olmasın iyi ve kötü siparişlerde bildirim seslerine benzer sesler yeterli olur".

- **Atıştırmalık sistemi (v5 P16) bütünüyle kaldırıldı.** Core: `SnackDefinition`, `TycoonRun`'daki kâse bölümü
  (`Snacks`, `SnackLeft`, `RestockSnacks`, `BuyBackTheBowls`, `ServeSnack`) ve kurucu parametresi, `CustomerVisit.SnacksTaken`
  ve `AddSnack`. Veri: `Assets/Data/snacks/` klasörü, `DataLoader.ParseSnacks` ve DTO'ları, `GameBootstrap.snacksJson`
  alanı ile sahnedeki referansı, `DebugSceneCreator`'ın yolu. Simülatör: her üçüncü serviste kâse kadansı, `SnackServes`
  ve rapor satırı. HUD: tezgâhtaki kâse sırası, eldeki kâse, tabureye tıklayınca servis, "tab + kâse" log satırı, imlecin
  kavrama koşulu. Testler: `ASnack_RidesTheTab_AndTheBowlRunsDown`, `ASnackAlone_IsRefused`, `SnacksFile_ParsesAndValidates`.
  Araçlar: `Tools/loc/data_keys.py`'nin `data.snack.*` üretimi; `bowl_down` klibi ve bankadaki tarifi. Yerelleştirme
  anahtarları (`data.snack.*`, `rule.snack_*`, `seats.snack.*`, `seats.log.tab_snacks`) evin kuralı gereği en.json'da ve
  diğer dillerde duruyor, artık kullanılmıyor. `prep_mat` ("Snack mat") fikstürü garnitür paspasıdır, kaldı. Kira böleni
  (GDD 23, 5.5) kâselerin marjı için sıkılmıştı; sim yeniden koşulmadan olduğu gibi bırakıldı. Dokümanlar: GDD 23 ve 22,
  `PLAN_service_depth` P16 notu, README, `SES_LISTESI`, `LAUNCH_READINESS`, `PLAN_localization_L1`, buradaki §6, §8, §9
  tabloları.
- **Barın uğultusu sustu:** §9.68'in `ambience_crowd` yatağı ve `Sfx.Ambience`'ın yatak seçimi gitti; pencereden yağmur
  tek ortam sesi. `ambience_loop` (eski sentetik müzik) yalnız hiç şarkı yokken çalan yedek olarak duruyor.
- **Sipariş sesleri bildirim:** `cheer_sfx` (iyi sipariş) ve `upset_sfx` (kötü) artık `music_synth.stinger`'dan — iyi
  sipariş iki hızlı parlak nota ve tepede elektrik piyano çınlaması, kötü sipariş minör üçlü inen iki yumuşak nota; insan
  sesi yok. Alkış ve "aww" kayıtları oyuna girmedi.
- Doğrulama: EditMode 595/595 (üç atıştırmalık testi silindi), PlayMode 13/13; kodda 'snack' yalnız garnitür paspasının adında ve kir çiziminin yorumunda kaldı.

### 9.69 · Onuncu geçiş: barın kendi synthwave'i, müzikal işaretler, üçüncü tur kayıtlar (2026-09-15)

Yazar (Pixabay'de MondaMusic'in "Synthwave Retro 80s" parçasını göstererek): "oyun müziği için bu tarzı uygun buldum bu tarz
şarkılar oluşturabilir misin_ Synthwave, Retro 80s, Synthwave retro music. Diğer seslerin hepsini beğendim oyuna entegre et.
oyunda başka eksik eski ses varsa onlara da alternatif sesler oluşturalım aynı şekilde"

- **Müzik oyunun kendi bestesi** (`Tools/music_synth.py`). Referans parça Content ID'ye kayıtlı olduğu için hiçbir parçası
  kullanılmadı; sayfasının etiketleri (synthwave, synth pop, synth disco, neon, gece, rahat, rüya gibi) tarif oldu. Hiç
  sample yok. Osilatörler toplamalı: her nota dalga biçiminin harmonikleri, "filtre" her harmoniğe 24 dB/oktav alçak
  geçiren yanıtın o frekanstaki kazancı — hareketli cutoff hareketli yanıt, aliasing yok. Enstrümanlar: LinnDrum tarzı
  davul (perdesi düşen kick, 80'lerin gated reverb'lü trampeti, clap, hi-hat, tom, crash), oktav pompalayan saw bas, üç
  sesli detune saw pad, noktalı sekizlik ping-pong delay'li kare arp, glide'lı saw lead, DX7 tipi FM elektrik piyano.
  Mix: kick bası, pad'i ve arp'ı sıkıştırıyor (sidechain); chorus pad'i ve piyanoyu genişletiyor; hall reverb ve delay
  send; master statik EQ, yavaş glue kompresör, bant saturasyonu ve bölümleri birbirine yaklaştıran yavaş seviye;
  dışa aktarımda −18 LUFS. scipy yok, her şey numpy ve FFT ile.
- **Yazım:** diyatonik yedili akorlar önceki akordan en az hareketle çevriliyor; lead iki taktlık bir motiften (A A' B
  A-son) anahtarın pentatonik dizisinde kuruluyor ve güçlü vuruşlarda akor seslerine oturuyor; bölümler (intro, verse,
  chorus, break, outro) enstrümanları açıp kapatıyor ve filtreleri giderek açıyor. Her şarkının tohumu sabit, her render
  aynı sesi veriyor.
- **Şarkılar (9):** gece `night_1` A minör 100 BPM (arp, saw lead), `night_2` F minör 108 BPM (synth disco: dört
  vuruşlu kick, elektrik piyano), `night_3` D dorian 94 BPM (piyano ağırlıklı, yumuşak lead), `night_4` E minör 104 BPM,
  `night_5` C# minör 98 BPM (A–B–C#m); son sipariş `lastcall_1` C minör 84 BPM (yumuşak davul); hikâye `story_1` G minör
  72 BPM (davulsuz, piyano ve pad); gün sonu `dayend_1` B♭ majör 92 BPM ve `dayend_2` F# minör 88 BPM. Her biri 2,5–3
  dakika. §9.68'in CC0 parçaları (Pro Sensory, HoliznaCC0, Some Weirdo) oyundan çıktı.
- **İlk render ölçülüp düzeltildi:** verse'lerde enerjinin %60'ı 60 Hz altındaydı (kick kuyruğu ve sub bas), intro ve
  break verse'lerden 11 dB kısıktı. Kick kısaltılıp en alt oktavı kesildi, bas kısıldı, hi-hat, pad, arp ve piyano açıldı,
  master'a yavaş seviye eklendi: sonra sub %27, alt %35, orta %32; bölümler 2,5 dB içinde.
- **Müzikal işaretler** (`music_synth.stinger`, `sfx_bank._cue`): `another_round`, `buy`, `order_ready`, `rim_done`, `serve_it`, `verdict_good`, `verdict_flat`, `verdict_bad`, `debt_alarm`, `synth_swell`, `cheer_sfx`, `upset_sfx` şarkıların enstrümanlarıyla çalıyor — iyi haber
  elektrik piyano, kötü haber yumuşak lead, borç alçak kare nabız, son müşteri sahnesi pad — ve bankın render'ından eski
  seviye ve odalarıyla çıkıyor. Eski sentetik tarifleri silindi.
- **Üçüncü tur kayıtlar** (20, CC0, `Docs/SES_KAYNAKLARI.md`): `bill_slip`, `dish_down`, `drain`, `glass_pickup`, `grain_pinch`, `head_settle`, `id_card_away`, `pour_floor`, `rim_turn`, `stir_loop`, `stir_commit`, `stool_take`, `tin_set_down`, `curtain`, `day_open`, `day_close`, `bar_closed`, `last_call_bell`, `screen_on`, `screen_off`. Kaynağıyla eşleşmesi zayıf kalan `blowout`,
  `garnish`, `pour_cutoff`, `tap_handle` ve `tin_tip` sentetik kaldı. `sfx_ingest` bir kesim modu daha kazandı: `window`
  (köpük ya da perde gibi vuruşsuz bir sesin en düzgün penceresi, döngüsüz).
- Şimdi: 68 klip CC0 kayıt, 12 müzikal işaret (§9.70'in iki sipariş sesi dahil), 11 sentetik klip (`ambience_loop`, `blowout`, `garnish`, `patience_warn`, `pour_cutoff`, `tap_handle`, `tin_tip`, `voice_greet`, `voice_happy`, `voice_order`, `voice_upset`).
- Doğrulama: EditMode 595/595, PlayMode 13/13 (görünüm testlerinde fark yok); şarkılar ve müzikal işaretler oyunda ölçüldü, konsol temiz.
...
- **Kart şişenin ARKASINDA oluşur** (yazarın notu, aynı gün): yuva rafta duran şişenin kendi yerine ve boyuna oturur (kopya
  ...
  köşelerinden merkez ve boyu okur; garnish kâseleri için aynı. **Yuvada kopya var, ama tek şişe** (yazarın son sözü: "açılan bilgi kartları her zaman hiyerarşide en üstte olmalı, sadece
  üstüne gelinen asset gözükecek … seçilen şey ışıklandırılmadan arındırılmış aydınlık şekliyle gözükecek"): kart HUD'un kendi
  kanvasında 26'da — raf yazılarının, kitabın, her sprite'ın üstünde. Yuvadaki şişe UI sandviçi (ışıksız = "aydınlık" hâli),
  `StepCellarCard` her kare raf şişesinin ön plakasının dünya köşelerini (`DiegeticStage.CellarBottleBounds`, sallanma dahil)
