# The star track — how far a bar climbs, and how fast

60 runs, horizon **120 days** (twenty weeks). The 200-run
report answers the same question over thirty days and is cut off by it.

**This is still a floor.** The bot reads only the ID and plays no better
than band midpoints — but since 2026-08-14 it DOES shop: stock its menu
names, the cheapest passing recipe, stools, glass steps, and one brand
UPGRADE a night behind a fat cushion. What the table is trusted for is the
SHAPE: the spacing between rungs, and where the curve stops moving. A rung
nobody reaches in twenty weeks is a rung no guest can be written for.

| Rung | Runs that reached it | Day p25/median/p75 | Median week |
|---|---|---|---|
| 0.0★ | 60 (100.0%) | 1 / 1 / 1 | 1 |
