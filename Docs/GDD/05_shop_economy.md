...
### 7.0 Baseline money rules (v1.1)
- **Starting money: $4.**
- Sell value of any owned item: ceil(buy price / 2), shown on the item.
- The shop never offers a Patron the player already owns; duplicate Patrons can only exist via copy effects (e.g., The Owner's Cat).
- Reroll cost resets to $5 at every new shop visit.
...
Examples: **Happy Hour** (+1 Restock), **Double Shift** (+1 Mix), **Wider Rail** (+1 Rail size), **Loyal Clientele** (Patrons $2 cheaper), **Neon Sign** (rarer Patrons appear more often), **Deep Cellar** (ingredient packs contain +1 card), **Bouncer** (once per Night, reroll the Night's VIP to a random valid one — respects no-repeat; VIP counterplay lever).

Rulings (matching implementation): every Back Room shows **one dedicated Voucher slot** offering a random voucher the player doesn't own yet; the slot is not affected by rerolls and each voucher can be owned once. Loyal Clientele's discount applies from the *next* shop (prices already on the table don't change) and never drops a patron below $1. **Neon Sign** doubles the Uncommon/Rare shop weights (base weights C60/U30/R10; Legendary weight 0 — "only via special means" = the Speakeasy Pack). **Deep Cellar** adds a 4th card to Cellar Packs. **Bouncer** is usable any time before the Night's VIP round starts. Launch pool of 7 lives in `Assets/Data/vouchers/vouchers.json`.
