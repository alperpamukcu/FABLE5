...
> **SUPERSEDED BY THE SHIPPED TREE (2026-09-13).** The live ladders, their tier order and their
> first-draft numbers are `Assets/Data/fixtures/fixtures.json`, generated by
> `Tools/upgrade_tree/ship.py` from the author's choices in the upgrade tree
> (`Tools/upgrade_tree/decisions.json`; GDD_MEVCUT §9.59). New since this table: a **ceiling** and a
> **floor** (7 rungs each, overlay layers at order 13), the back wall at 5 rungs (chevron at 4),
> the right wall at 7, the rug at 6, the wall lamps at 6, the neon at 5, the centre picture at 5,
> the **screen & posters** at 8 (the set first), a **picture over the screen** at 4; the bar opens
> with the bistro tables (0 comfort); fiddle, snake and table rungs 1–2 left the game. The TOOLS
> (tower, tin, basin) are only ever upgraded and each rung works faster (`workSpeed`, `washSeconds`).
> A shipping test holds every ladder to costing more, being worth more and asking no fewer stars
> as it climbs. The table below is the v1 history.
...
| `walls` (the back wall) | cracked plaster (ours) · 0 | fresh plaster **$45** · **+1.25 (0★)** | panelled $130 · **+2.25 (1.0★)** | **FOUR rungs, the author's own plates (2026-09-06)** — rung 4 is the harlequin paper, $200 · **+3.25 (2.0★)**. **THE WALLS CARRY THE ROOM** (2026-09-06, the author: *"duvar geliştirmeleri konforu en çok arttıran geliştirmeler olmalı ... markette walls upgrade'i daha çok göze sokulmalı"*): with the free base at 0, the second rung is ungated and alone reaches the first star, and each rung's gate sits under the comfort the rung before it gives. The market's walls shelf is the first shelf, and until rung 2 is up its sign is amber: START HERE ▸ THE WALLS — THE ROOM'S COMFORT; every comfort-bearing tile says its figure (COMFORT +1.25) where the fitting's mark used to be. **TWO SHELVES, AND THE DEAREST THING IN THE SHOP (2026-09-06, the author: "duvar geliştirmeleri en
pahalı en büyük geliştirme olmalı. Duvar geliştirmeleri ile duvara yapılan geliştirmeler ayrı
olmalı"):** the plaster ladder is THE WALLS; the triptych and the screen moved to a `wall_art` group
— ON THE WALL — sold right after it. Under the star economy (GDD 23) the ladder's rungs are priced
at their stages: $45 · $260 · $600 — the top rung the most expensive thing in the catalogue. The
snack mat is a FIXTURE now (`prep_mat`, given with the room like the drip mat, slot at 258/69), lit
by the room's lights instead of tinted by hand — and it GROWS WITH THE RAIL (2026-09-06, the author:
"yeni garnish geldiğinde de ona göre boyutu sağ ve sola doğru uzamalı"): the HUD tells the room the
standing dishes' span (`DiegeticStage.SetPrepMatSpan`) and the mat is tiled to it and re-centred on
it; its slot keeps only the height it stands at. **MEASURED (§7, 2026-09-06):** the bare start is a real cost. With the bot buying the plaster first (it reads the same START HERE sign), the 200-run floor went from 0% to **34% bankruptcies** at $45, 68% at the old $70, 27% at $40 (which the ladder pin forbids: a rung costs more than the one under it); cutting rungs 3–4 to $90/$150 changed nothing, since the bot never holds $150. The binding constraint is not the price but the standing's climb from zero (0.125 a night toward the walls' 1.25, 1.0★ on day 11 instead of day 7) under the door's zero-star arrival gap (GDD 23 §7) — the author's call whether that opening is the game or the door's rule softens. A rung of this ladder is the whole 640×360 room (`backdrop` slot: the sprite REPLACES the plate, nothing stands at a hook), so the market tile shows a 64×48 `swatch` of the wall instead of the room shrunk. The bar opens in the cracked room; the door's sign reads +20 ONLY on every plate. |
...
- A rung is bought one at a time (`CanBuyRung`: `Level == LadderLevel(slot) + 1`); the lower rung
  stays owned. ~~The market shows the owned rung and the NEXT one only (2026-08-26)~~ — **every rung
  is shown since 2026-09-13** (*"tüm seçenekler desen png si ile gözükmeli"*), the ones past the next
  locked with the gate they wait on; and since 2026-09-12 the room draws the rung the player WEARS
  (any owned one), while comfort reads the tallest (§5).
...
- **The upgrade screen, 2026-09-06 (the author: "upgrade kısmını güzelce gruplandır karışık
  ...
  upgrade and stays.
- **The decor catalogue, 2026-09-13 (the author: "Her bölümün ayrı kısmı olmalı markette. Örneğin
  duvar dendiğinde tüm seçenekler desen png si ile gözükmeli ... Sims'in dekorasyon UI'ı güzeldi").**
  The UPGRADES department stands a RAIL of shelves left of the aisle — BAR & GLASS (stools, the bar
  top, glass lines), WALLS, ON THE WALL, LIGHTS, FLOOR, PLANTS, COUNTER — each key with its group's
  pictogram and a count of what could be bought tonight. The open shelf shows each LADDER as a head
  (its name, MARK n OF N and what the top rung is worth) and a row of 152×204 cards, one per rung,
  EVERY rung: its swatch or drawing in the window, its mark in the corner, and one control — WORN
  (green plate), WEAR (bought; free; comfort stays at the climbed rung), price + ADD (the next rung),
  or LOCKED (dimmed, MARK n FIRST or the star gate). This overturns two rules above: owned pieces
  are no longer taken off the shelf (you wear from it) and rungs past the next are no longer hidden.
  The WHAT THE ROOM WEARS shelf (2026-09-12) and the department's "n more waiting" crate folded
  into the cards. `TycoonHud.Decor.cs`.
- Tables became ladders in data (§3.1).
