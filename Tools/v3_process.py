...
def sandwich(master, mouth):
    ...
    for y in range(H):
        ...
        for x in xs:
            ...
            c = px[x, y][:3]
            # Film back at 70 — the author's chosen reference state ("ayarlar 2.
            # pushuna dönsün") carries THIS glass, and the 44 thinning that slipped
            # through the integration re-bakes is what made the near-opaque liquid
            # read as paint ON the bottle ("artık daha fazla önüne geçmiş",
            # 2026-08-05). The glass has to sit visibly OVER the drink to put the
            # drink inside. Bright streaks outside the label boxes ride at 166.
