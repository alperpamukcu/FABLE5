...
def peel(im, layers=OUTLINE):
    """Take the existing ink ring off, one layer at a time, and no deeper."""
    out = im.copy()
    p = out.load()
    W, H = out.size
    for _ in range(layers):
        ...
        for x in range(W):
            for y in range(H):
                if p[x, y][3] < 128 or lum(p[x, y][:3]) > DARK:
                    continue
                edge = any(not (0 <= x + dx < W and 0 <= y + dy < H) or p[x + dx, y + dy][3] < 128
...
def run(write):
    ...
    for style in styles():
        ...
        if loaded[0] is None:
            continue
        peeled = [peel(im) if im is not None else None for im in loaded]
