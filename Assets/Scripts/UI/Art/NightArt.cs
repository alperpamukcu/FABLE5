...
namespace LastCall.UI
{
    ...
    /// için pixelart oluşturulsun eğer beğenilmezse sabit düz desenli bir arkaplan"). The pause menu's backdrop, the
    /// plates the menus and the hover tips stand on, and the marks their keys carry — all drawn here from the palette,
    /// pixel by pixel, because UI chrome is never generated in this project (GDD 14 §2; ChromeArt's rule). Everything
    /// is cached the way ChromeArt caches: a sprite made in play mode dies with it, and a dead sprite is not a hit.
    ///
    /// The backdrop is drawn at HALF size and shown at exactly 2x, which keeps it pixel art (16 §3): a 640x360 night —
    /// stars, a banded sun, a far city with its windows lit, the sun on the water, the grid road, and palms — under the
    /// scanlines the menu wears. <see cref="UseFlatBackdrop"/> swaps it for the flat pattern the author asked for as
    /// the fallback, one line.
    /// </summary>
    public static class NightArt
    {
        ...
        /// <summary>The drawn night, 640x360, to be shown at 2x. Or the flat pattern, when <see cref="UseFlatBackdrop"/>.</summary>
        public static Sprite Backdrop() => UseFlatBackdrop ? FlatPattern() : PalmNight();

        private static Sprite PalmNight()
        {
            const string Key = "night:palm";
            if (Cache.TryGetValue(Key, out var got) && got != null) return got;
            const int W = 640, H = 360, Horizon = 215;
            ...
            for (int i = 0; i < 160; i++)
            {
                int x = rng.Range(W), ty = rng.Range(150);
            ...
            const int Cx = 320, Rad = 90;
            int sunFoot = Horizon - 4;
            foreach (var (rr, col) in new[] { (140, UITheme.Magenta[0]), (118, UITheme.Magenta[1]) })
            ...
            for (int row = 0; row < 2; row++)
            {
                ...
                while (x < W + 20)
                {
                    int w = 12 + rng.Range(5) * 4, h = (row == 0 ? 20 : 45) + rng.Range(row == 0 ? 15 : 30) * 2;
            ...
            for (int y = Horizon; y < Horizon + SeaH; y += 2)
            {
                ...
                int half = Mathf.RoundToInt(30f + t * 70f);
            ...
            for (int k = 0; k < 14; k++) Box(0, floorTop + Mathf.RoundToInt(k * k * 1.1f), W, 1, C(UITheme.Cyan[2]));
            for (int i = -16; i <= 16; i++)
            {
                int x1 = Cx + i * 75;
            ...
            foreach (var (bx, dir, small) in new[] { (60, 1, false), (580, -1, false), (20, 1, true), (620, -1, true) })
            {
                int hgt = small ? 120 : 170, lean = small ? 15 : 35;
                var ink = C(UITheme.Night[0]);
                for (float t = 0f; t < 1f; t += 0.02f)
                {
                    int x = Mathf.RoundToInt(bx + dir * lean * t * t), y = Mathf.RoundToInt(floorTop + 15 - t * hgt);
                    Box(x - 3 + Mathf.RoundToInt(t * 1.5f), y, 7 - Mathf.RoundToInt(t * 3f), 3, ((int)(t * 50f)) % 2 == 0 ? ink : C(UITheme.Night[1]));
                }
                int tx = bx + dir * lean, ty = floorTop + 15 - hgt;
                for (int f = 0; f < 7; f++)
                {
        ...
        public static Sprite MenuPlate()
        {
            const string Key = "night:plate";
            if (Cache.TryGetValue(Key, out var got) && got != null) return got;
            const int S = 24;
            var px = new Color32[S * S];
            var fill = C(UITheme.Night[1]); fill.a = 232;
