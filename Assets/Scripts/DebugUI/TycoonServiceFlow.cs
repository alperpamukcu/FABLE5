...
namespace LastCall.DebugUI
{
    ...
    public sealed class TycoonServiceFlow : MonoBehaviour
    {
        ...
        private bool _pourHeld;             // hold-to-pour into the shaker
        ...
        private void Update()
        {
            ...
            if (_stage == Stage.Shaker && _pourHeld && _focusBottle != null &&
                _focusBottle.Type != IngredientType.Garnish)
            {
                run.PourTick(Time.deltaTime);
                if (run.PouringId == null) _pourHeld = false;
                RefreshShaker();
            }
        ...
        private void BuildShakerPanel()
        {
            ...
            Place(_shakerPanel, new Vector2(0.5f, 0.5f), new Vector2(560, 420), Vector2.zero);
            ...
            Stretch(_shakerTitle.rectTransform, new Vector2(0, 1), Vector2.one, new Vector2(0, -48), new Vector2(0, -10));

            var zone = NewRect("PourZone", _shakerPanel);
            Place(zone, new Vector2(0.5f, 0.5f), new Vector2(360, 200), new Vector2(0, 20));
            var zoneImg = zone.gameObject.AddComponent<Image>();
            zoneImg.color = new Color(UITheme.Amber[1].r, UITheme.Amber[1].g, UITheme.Amber[1].b, 0.5f);
            HoldTrigger(zone, () => _pourHeld = HoldStartPour(), () => { _pourHeld = false; Run.EndPour(); });
            var zoneLabel = NewText("Label", zone, _display, 14, TextAnchor.MiddleCenter, UITheme.TextPrimary);
            Stretch(zoneLabel.rectTransform, Vector2.zero, Vector2.one, Vector2.zero, Vector2.zero);
            zoneLabel.text = "HOLD TO POUR";

            _shakerReadout = NewText("Readout", _shakerPanel, _body, 13, TextAnchor.LowerCenter, UITheme.TextSecondary);
            Stretch(_shakerReadout.rectTransform, Vector2.zero, new Vector2(1, 0), new Vector2(16, 52), new Vector2(-16, 90));

            var back = NewRect("Back", _shakerPanel);
            Place(back, new Vector2(0.5f, 0), new Vector2(200, 34), new Vector2(0, 12));
            back.gameObject.AddComponent<Image>().color = UITheme.PrimaryAction;
            var backBtn = back.gameObject.AddComponent<Button>();
            backBtn.onClick.AddListener(() => GoTo(Stage.Menu));
        ...
        private bool HoldStartPour()
        {
            if (_focusBottle == null || Run.Phase != TycoonPhase.DayOpen) return false;
            Run.BeginPour(_focusBottle.Id);
            return true;
        }
